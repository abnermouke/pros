<div class="d-flex flex-column {{ $hidden ? 'd-none' : '' }} pros_form_{{ $sign }}_item_box" data-target="#pros_form_{{ $sign }}_item_{{ $field }}" data-required="{{ $required ? 1 : 0 }}" data-type="{{ $type }}" data-field="{{ $field }}" data-default-value="#pros_form_{{ $sign }}_item_{{ $field }}_default_value_content" data-upload-url="{{ $upload_url }}">
    ...
    <textarea class="form-select form-select-solid" autocomplete="off" id="pros_form_{{ $sign }}_item_{{ $field }}" placeholder="{{ isset($placeholder) ? $placeholder : '' }}" rows="{{ isset($row) ? (int)$row : 3 }}" {!! isset($max_length) && (int)$max_length > 0 ? 'maxlength="'.$max_length.'"' : '' !!} {{ $readonly ? 'readonly' : '' }} {{ $disabled ? 'disabled' : '' }} name="{{ $field }}">{!! $default_value !!}</textarea>
    @if($description)
